...
visitor rather than a test ever runs. Sixteen files and a directory of pictures,
...
| `menu.ts` | Phase 6.3. `IDR_MENU` drawn — the popup, the accelerator text, the gutter bitmaps, and the right-justified caption. Anything with no handler is `MF_GRAYED` rather than silent, though since 8.2 and 8.4 nothing is. Since 7.1 it also binds `IDR_ZKRATKY`, by the same rule: five of those commands (F7, F8, the two with Ctrl, and Ctrl+Shift+Alt+D) are in no menu at all. What it draws is `exhibitMenu(IDR_MENU)`, not the resource itself — see `caption.ts`. |
| `caption.ts` | Phase 6.3, and one of the files here that spell Czech by hand: `ZAPIS_DO_MENU_AKTUALNI_STAV_NASTAVENI` builds the status line with `strcat` out of two switches, so there is no resource to read its fourteen words from. `node test/app/caption.test.ts` finds every one of them in `PROSTRED.FU` as CP1250 bytes. Phase 8.2 added `exhibitMenu` and `DROPPED_COMMANDS`: **the one place this port rearranges the author's own menu**, and the argument for each of the three commands it drops is written over it. |
...
| `dialog.ts` | Phase 7.1. `IDD_NASTAVENI` drawn — the list boxes, the radios, the name edits, in `frame.ts`'s window. Unlike the main window, **the layout of this one is entirely the resource script's**: a modal dialog is laid out by `MapDialogRect` and nothing moves afterwards, so there is not one measurement in the file. `DROPPED` is the cut: the second page, the two buttons that switched to it, and — since phase 8 — the conversation log's own tick. Whatever a drop leaves behind closes up: `droppedRows` moves everything below a dropped band up by its height and shortens anything that spanned it, which is why "Prostředí" has no hole in it. |
| `settings.ts` | Phase 7.1, and another file here that spells Czech by hand — four strings `Nastaveni.cpp` holds as literals. `CNastaveni` with the window taken off: his `OnInitDialog`, his `OnOK`, `ZKONTROLUJ_SPRAVNOST_ZNAKU_VE_JMENE`, and the two lists of control ids his two pages showed (the second of which `dialog.ts` no longer draws). `node test/app/settings.test.ts` parses all of it back out of `Nastaveni.cpp` and compares. |
...
| `frame.ts` | Phase 8.2, and an extraction rather than a new idea. All four of his modal dialogs are the same `DS_MODALFRAME | WS_CAPTION | WS_SYSMENU` popup, so the frame phase 7.1 had to invent for one of them lives here: the XP caption bar, the close box carrying `IDCANCEL`'s own name, Escape, the Tab trap, and `MapDialogRect` placement. It also holds `DIALOG_FONT`, `renderLabel` and `makeStatic`, which were three copies waiting to happen. |
| `screens.ts` | Phase 8.2. `IDD_TEXT` (twice: Malá nápověda and Informace o verzi) and `IDD_ABOUTBOX`, drawn. The interesting half is the rich edit: `NAPIS_FORMATOVANY_TEXT_NAPOVEDY` reads four tags out of the text and sends `EM_SETCHARFORMAT` per run, and each of the four becomes one CSS declaration with his own argument beside it. |
| `help.ts` | Phase 8.2, and the file here with the most of the author's Czech in it: the three screens that are only words — `ZOBRAZ_NAPOVEDU`, `OnOverzi` and the about box's paragraph of thanks — plus `markup()`, the parsing half of his rich-edit formatter. `node test/app/help.test.ts` reassembles all three out of his own C string literals, splice for splice, and compares them for a man and for a woman. |
| `cheat.ts` | Phase 8.4. `IDD_DEBUGNASTAVENI` drawn, and **the only one of his four dialogs this port draws whole** — nothing dropped, nothing greyed. Reachable by Ctrl+Shift+Alt+D or by typing `::debuginfo`, both of them his. It carries the only tool tips in the port: twelve paragraphs out of `BublinkovaNapoveda`, shown when `zobrazovatpopisky` is 1, which is his own condition. |
| `debug.ts` | Phase 8.4. `CDebugNastaveni` with the window taken off: the report, the two radio vocabularies, `OnOK` with both refusals and the one `MB_OKCANCEL` question, and the twelve tool tips. `node test/app/debug.test.ts` parses his `sprintf`, his `CheckDlgButton` switches and his `AddTool` calls back out of `debugnastaveni.cpp`. |
